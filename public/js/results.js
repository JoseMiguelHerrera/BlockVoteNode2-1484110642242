...
function addElectionQuestion(question) {
	$("#electionQuestion").html(question);
}
...
function multAjaxCallResults(count, finalCount) {
	if (count < finalCount) {
		$.ajax({
			url: baseUrl + "/getElectionInfo",
			success: function (resp) {

				addElectionQuestion(resp.response.electionData.electionQuestion)

			}, error: function () {
				console.log("error geting election info")
			}
		})
